...
def test_feature_extraction(conversation_id: int):
    ...
    if not conv:
        print(f"[ERROR] 对话 ID={conversation_id} 不存在")
        return
    ...
    try:
        ...
        print(f"[OK] 特征提取完成！耗时: {elapsed_time:.2f}秒")
        print(f"  任务ID: {result['task_id']}")

        # 3. 显示会话切分结果
        print_section("2. 会话切分结果")
        sessions = result['sessions']
        print(f"[OK] 检测到 {len(sessions)} 个会话")
        ...
        # 4. 显示响应时间统计
        print_section("3. 响应时间统计")
        rt_stats = result['response_time_stats']
        print(f"  有效响应次数: {rt_stats['count']}")
        if rt_stats['avg']:
            print(f"  平均响应时间: {rt_stats['avg']:.1f}秒 ({rt_stats['avg']/60:.1f}分钟)")
        else:
            print(f"  平均响应时间: 无数据")
        if rt_stats['median']:
            print(f"  中位数: {rt_stats['median']:.1f}秒 ({rt_stats['median']/60:.1f}分钟)")
        else:
            print(f"  中位数: 无数据")
        if rt_stats['min']:
            print(f"  最快: {rt_stats['min']:.1f}秒")
        else:
            print(f"  最快: 无数据")
        if rt_stats['max']:
            print(f"  最慢: {rt_stats['max']:.1f}秒 ({rt_stats['max']/60:.1f}分钟)")
        else:
            print(f"  最慢: 无数据")
        print(f"  异常响应: {rt_stats['abnormal_count']}个")

        # 5. 显示主动性统计
        print_section("4. 主动性统计")
...
if __name__ == "__main__":
    print("="*60)
    print("  Chrono Trace - Feature Extraction Test")
    print("="*60)
    ...
    if result:
        ...
        print("[WARN] 没有找到测试数据（需要消息数>=50的对话）")
