...
def _resolve_event_context(conn, suggestion_id: int) -> dict[str, Any]:
    ...
    shown_row = conn.execute(
        """
        SELECT batch_id, display_name, trigger_type
    ...
    *,
    ...
    if event_type in SINGLETON_EVENT_TYPES:
        conn.execute(
            ...
            (suggestion_id, batch_id, display_name, trigger_type, event_type,
...
def sweep_ignored_suggestions(
    ...
    *,
    ...
    rows = conn.execute(
        f"""
        SELECT s.id, shown.batch_id, shown.display_name, shown.trigger_type
        ...
        """,
        (EVENT_SHOWN, *TERMINAL_EVENT_TYPES, cutoff),
    ).fetchall()
